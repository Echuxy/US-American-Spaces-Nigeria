...
export default function SummitContentManager() {
  ...
  const saveProfile = async () => {
    ...
    if (headshot) {
      if (headshot.size > MAX_HEADSHOT_MB * 1024 * 1024) return setProfileError(`Headshot is larger than ${MAX_HEADSHOT_MB} MB.`)
      if (!headshot.type.startsWith('image/')) return setProfileError('Headshot must be an image file.')
      const safe = headshot.name.replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/-+/g, '-')
      const path = `facilitators/${session.day_id}/${session.id}/${Date.now()}-${safe}`
      const { error: uploadError } = await summitSupabase.storage
        .from('summit-presentations')
        .upload(path, headshot, { contentType: headshot.type, upsert: false })
      if (uploadError) return setProfileError(uploadError.message)
      headshotUrl = summitSupabase.storage.from('summit-presentations').getPublicUrl(path).data.publicUrl
    }

    setSavingProfile(true)
    const { error } = await summitSupabase
      .from('sessions')
      .update({ facilitator_bio: bio.trim() || null, facilitator_headshot_url: headshotUrl })
      .eq('id', session.id)
    if (error) {
      setProfileError(error.message)
    ...
    setSessions(items => items.map(item => item.id === session.id
      ...
      : item))
    setHeadshot(null)
    setProfileMessage('Facilitator profile saved.')
    setSavingProfile(false)
  }
  ...
  return <div style={{minHeight:'100vh',background:'#f3f6fa',fontFamily:'Inter,system-ui,-apple-system,"Segoe UI",sans-serif',color:'#172337'}}>
    ...
    <main style={{maxWidth:1100,margin:'20px auto',padding:'0 16px'}}>
        ...
        <section style={{marginTop:15,background:'#fff',border:'1px solid #dce4ed',borderRadius:14,overflow:'hidden'}}>
          ...
          <div style={{padding:16,display:'grid',gridTemplateColumns:'minmax(0,1fr) 240px',gap:16}}>
            <div>
              ...
              <button onClick={saveProfile} disabled={savingProfile} style={{marginTop:12,border:0,borderRadius:8,padding:'9px 13px',background:'#173b68',color:'#fff',fontSize:11,fontWeight:700,cursor:savingProfile?'wait':'pointer',opacity:savingProfile?.6:1}}>{savingProfile?'Saving…':'Save Facilitator Profile'}</button>
            ...
            <div>
              ...
              {session.facilitator_headshot_url
                ? <img src={session.facilitator_headshot_url} alt={session.facilitator || 'Facilitator'} style={{display:'block',width:'100%',aspectRatio:'1 / 1',objectFit:'cover',borderRadius:10,marginTop:7,border:'1px solid #dce4ed'}} />
      ...
      <div style={{marginTop:15,padding:14,background:'#fff',border:'1px solid #dce4ed',borderRadius:14,fontSize:11,color:'#63748a',lineHeight:1.6}}><strong>Recommended workflow:</strong> enter the approved facilitator bio and headshot first, then upload the PDF version for live synchronized presentation. Upload the original PPT/PPTX as a downloadable source when required. Maximum presentation file size is 50 MB.</div>
    </main>
  </div>
}
