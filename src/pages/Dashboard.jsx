...
try {
  ...
  const isFiltered = filterSpace || filterStatus || filterPillar || filterPriority || filterDateFrom || filterDateTo || search

  return (
    <div style={s.page}>
      {/* Announcement popup */}
      {selectedAnnouncement && (
        <div style={s.overlay} onClick={() => setSelectedAnnouncement(null)}>
          <div style={s.annModal} onClick={e => e.stopPropagation()}>
            <div style={s.annModalHeader}>
              <div>
                {selectedAnnouncement.is_pinned && <span style={s.pinTag}>📌 Pinned</span>}
                <h3 style={s.annModalTitle}>{selectedAnnouncement.title}</h3>
                <p style={s.annModalDate}>
                  {new Date(selectedAnnouncement.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' })}
                  {selectedAnnouncement.expires_at && ` · Expires ${new Date(selectedAnnouncement.expires_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short' })}`}
                </p>
              </div>
              <button style={s.closeBtn} onClick={() => setSelectedAnnouncement(null)}>✕</button>
            </div>
            <div style={s.annModalBody}>
              <p style={s.annModalText}>{selectedAnnouncement.body}</p>
            </div>
            <div style={s.annModalFooter}>
              <button style={s.viewAllBtn} onClick={() => { setSelectedAnnouncement(null); navigate('/announcements') }}>
      ...
      <div style={s.topbar}>
        ...
        <div style={s.topbarRight}>
          <span style={s.userBadge}>{profile?.full_name} · <em>{profile?.role?.replace('_', ' ')}</em></span>
          {(profile?.role === 'space_director' || profile?.role === 'admin') && (
            <button style={s.newReportBtn} onClick={() => navigate('/report/new')}>+ New Report</button>
          )}
      ...
      <div style={s.body}>
        ...
        {(
          <div style={s.annSection}>
            <div style={s.annSectionHeader}>
              ...
              <button style={s.annViewAll} onClick={() => navigate('/announcements')}>View All →</button>
            </div>
            <div style={s.annGrid}>
              {announcements.map(ann => (
                <div key={ann.id} style={{
                  ...s.annTile,
                  borderTop: ann.is_pinned ? '3px solid #B22234' : '3px solid #3C3B6E',
                }} onClick={() => setSelectedAnnouncement(ann)}>
                  {ann.is_pinned && <span style={s.pinTag}>📌 Pinned</span>}
                  <h4 style={s.annTileTitle}>{ann.title}</h4>
                  <p style={s.annTileBody}>{ann.body?.slice(0, 80)}{ann.body?.length > 80 ? '...' : ''}</p>
                  <p style={s.annTileDate}>
        ...
        <div style={s.statGrid}>
          ...
          {canReview && <StatCard icon="⏳" label="Awaiting Your Review" value={pendingReview} color={pendingReview > 0 ? '#d97706' : '#6b7280'} />}
        </div>
        ...
        <div style={s.chartsRow}>
          <div style={s.chartCard}>
            ...
            {pillarCounts.length === 0 ? <p style={s.empty}>No data yet.</p>
              : pillarCounts.map(p => (
                <div key={p.label} style={s.barRow}>
                  <span style={s.barLabel}>{p.label}</span>
                  <div style={s.barTrack}><div style={{ ...s.barFill, width: `${(p.count / maxPillar) * 100}%`, background: 'linear-gradient(90deg, #3C3B6E, #B22234)' }} /></div>
            ...
            {priorityCounts.map((p, i) => {
              ...
              return (
                <div key={p.label} style={s.priorityRow}>
                  ...
                  <span style={{ ...s.priorityCount, background: colors[i] + '22', color: colors[i] }}>{p.count}</span>
                </div>
              )
            })}
          </div>
        ...
        <div style={s.filterCard}>
          <div style={s.filterRow}>
            ...
            <select style={s.filterInput} value={filterPillar} onChange={e => setFilterPillar(e.target.value)}>
              <option value="">All Pillars</option>
              {PILLARS.map(p => <option key={p.id} value={p.label}>{p.label}</option>)}
            </select>
            <select style={s.filterInput} value={filterPriority} onChange={e => setFilterPriority(e.target.value)}>
              <option value="">All Priorities</option>
              {STRATEGIC_PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div style={s.filterRow2}>
            ...
            {isFiltered && <button style={s.clearBtn} onClick={clearFilters}>✕ Clear Filters</button>}
        ...
        <div style={s.tableCard}>
          {loading ? <p style={s.empty}>Loading reports...</p>
            : filtered.length === 0 ? <p style={s.empty}>No reports found. {isFiltered && 'Try clearing the filters.'}</p>
            : (
              <div style={s.tableWrap}>
                <table style={s.table}>
                  <thead>
                    <tr>{['Date', 'Space', 'Programme Title', 'Pillar', 'Attendance', 'Amount (₦)', 'Status', 'Action'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {filtered.map((r, i) => {
                      ...
                      return (
                        <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9fafb' }}>
                          ...
                          <td style={{ ...s.td, maxWidth: '200px' }}><span style={s.titleCell}>{r.programme_title}</span></td>
                          <td style={s.td}><span style={s.pillarTag}>{r.pillar}</span></td>
                          <td style={{ ...s.td, textAlign: 'center' }}>{r.attendance}</td>
                          <td style={{ ...s.td, textAlign: 'right' }}>{Number(r.amount_spent).toLocaleString()}</td>
                          <td style={s.td}><span style={{ ...s.statusBadge, background: st?.color }}>{st?.label}</span></td>
                          <td style={s.td}>
                            <button style={{ ...s.viewBtn, ...(isPending ? s.viewBtnUrgent : {}) }} onClick={() => navigate(`/report/${r.id}`)}>
...
function StatCard({ icon, label, value, color }) {
  return (
    <div style={s.statCard}>
      ...
      <div><p style={s.statValue}>{value}</p><p style={s.statLabel}>{label}</p></div>
...
const s = {
  ...
  topbar: {
    background: 'linear-gradient(135deg, #1a1f3a, #2d3561)', padding: '8px 16px',
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    flexWrap: 'wrap', gap: '8px', position: 'sticky', top: 0, zIndex: 100,
    boxShadow: '0 2px 12px rgba(0,0,0,0.3)',
  },
  topbarLeft: { display: 'flex', alignItems: 'center', gap: '16px' },
  flagStripe: { width: '6px', height: '36px', borderRadius: '3px', background: 'linear-gradient(180deg, #B22234 33%, #fff 33%, #fff 66%, #3C3B6E 66%)' },
  ...
  newReportBtn: { padding: '7px 14px', background: 'linear-gradient(135deg, #B22234, #8b1c2a)', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' },
  navBtn: { padding: '7px 12px', background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap' },
  signOutBtn: { padding: '7px 12px', background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' },
  body: { padding: '20px', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' },
  // Announcements
  annSection: { background: '#fff', borderRadius: '12px', padding: '16px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' },
  annSectionHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' },
  annSectionTitle: { margin: 0, fontSize: '15px', fontWeight: 700, color: '#1a1f3a' },
  annViewAll: { background: 'none', border: 'none', color: '#B22234', fontSize: '13px', fontWeight: 600, cursor: 'pointer' },
  annGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '12px' },
  annTile: {
  ...
  annTileTitle: { margin: 0, fontSize: '13px', fontWeight: 700, color: '#1a1f3a', lineHeight: 1.4 },
  annTileBody: { margin: 0, fontSize: '12px', color: '#6b7280', lineHeight: 1.5 },
  annTileDate: { margin: 0, fontSize: '11px', color: '#9ca3af' },
  pinTag: { fontSize: '10px', background: '#fef2f2', color: '#B22234', padding: '2px 7px', borderRadius: '20px', fontWeight: 700, alignSelf: 'flex-start' },
  // Announcement modal
  ...
  annModal: { background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '560px', maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' },
  annModalHeader: { background: 'linear-gradient(135deg, #1a1f3a, #2d3561)', padding: '18px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' },
  annModalTitle: { margin: '4px 0 0', color: '#fff', fontSize: '16px', fontWeight: 700 },
  annModalDate: { margin: '6px 0 0', color: '#93a4d4', fontSize: '12px' },
  closeBtn: { background: 'none', border: 'none', color: '#fff', fontSize: '18px', cursor: 'pointer' },
  annModalBody: { padding: '20px', overflowY: 'auto', flex: 1 },
  annModalText: { fontSize: '14px', color: '#374151', lineHeight: 1.8, margin: 0, whiteSpace: 'pre-wrap' },
  annModalFooter: { padding: '14px 20px', borderTop: '1px solid #f3f4f6' },
  viewAllBtn: { width: '100%', padding: '10px', background: 'linear-gradient(135deg, #B22234, #3C3B6E)', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' },
  // Stats
  ...
  filterInput: { padding: '8px 12px', border: '1.5px solid #d1d5db', borderRadius: '8px', fontSize: '13px', outline: 'none', flex: '1 1 160px', minWidth: '140px', fontFamily: "'Segoe UI', sans-serif" },
  dateLabel: { fontSize: '12px', color: '#6b7280', fontWeight: 600 },
  clearBtn: { padding: '8px 14px', background: '#fef2f2', border: '1px solid #fca5a5', color: '#dc2626', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: 600 },
  ...
  pillarTag: { background: '#eef0ff', color: '#3C3B6E', padding: '3px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, whiteSpace: 'nowrap' },
  statusBadge: { padding: '4px 10px', borderRadius: '20px', color: '#fff', fontSize: '11px', fontWeight: 700, whiteSpace: 'nowrap' },
  viewBtn: { padding: '6px 14px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: 600, color: '#374151' },
  viewBtnUrgent: { background: 'linear-gradient(135deg, #B22234, #3C3B6E)', color: '#fff', border: 'none' },
