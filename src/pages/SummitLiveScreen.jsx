...
export default function SummitLiveScreen(){
  ...
  const [live,setLive]=useState({dayId:'day1',sessionIndex:1,slide:1,displayMode:'program',emergencyMessage:'',onAir:true})
  ...
  useEffect(()=>realtime.subscribe(event=>{
    ...
    if(type===summitEventTypes.LIVE_STATE_CHANGED) setLive({dayId:payload.dayId,sessionIndex:Number(payload.sessionIndex),slide:Number(payload.slide||1),displayMode:payload.displayMode||'program',emergencyMessage:payload.emergencyMessage||'',onAir:payload.onAir!==false})
  ...
  useEffect(()=>{ if(!summitSupabase)return; (async()=>{ const {data}=await summitSupabase.from('summit_live_state').select('day_id,session_index,slide,display_mode,emergency_message,on_air').eq('id',1).maybeSingle(); if(data)setLive({dayId:data.day_id,sessionIndex:Number(data.session_index),slide:Number(data.slide||1),displayMode:data.display_mode||'program',emergencyMessage:data.emergency_message||'',onAir:data.on_air!==false}) })() },[])
  ...
  .live-screen{width:100vw;height:100vh;overflow:hidden;background:#0d1b2e;color:#fff;font-family:Inter,system-ui,-apple-system,"Segoe UI",sans-serif;position:relative}.live-top{position:absolute;z-index:4;top:0;left:0;right:0;padding:22px 34px;display:flex;justify-content:space-between;align-items:center;background:linear-gradient(180deg,rgba(8,22,38,.92),transparent)}.eyebrow{font-size:11px;letter-spacing:.16em;color:#b7cbe0;font-weight:800}.live-brand{font-size:12px;font-weight:800;letter-spacing:.08em}.live-main{height:100%;display:flex;align-items:center;justify-content:center;padding:72px 34px 66px;box-sizing:border-box}.deck{width:min(94vw,1500px);height:min(78vh,820px);background:#162a43;border:1px solid rgba(169,196,225,.16);border-radius:14px;overflow:hidden;box-shadow:0 28px 80px rgba(3,16,32,.38);position:relative}.deck iframe{width:100%;height:100%;border:0;background:#fff}.fallback-slide{height:100%;display:flex;flex-direction:column;justify-content:center;padding:8vw;box-sizing:border-box;background:radial-gradient(circle at 70% 35%,rgba(67,157,225,.24),transparent 45%),linear-gradient(135deg,#122842,#0b1728)}.fallback-slide .number{font-size:13px;color:#78a9d1;letter-spacing:.16em;font-weight:800}.fallback-slide h1{font-size:clamp(36px,5vw,84px);line-height:.98;max-width:1100px;margin:18px 0}.facilitator{font-size:18px;color:#b9c8d9}.bottom{position:absolute;z-index:5;left:0;right:0;bottom:0;padding:14px 34px;display:flex;justify-content:space-between;gap:20px;align-items:center;background:linear-gradient(0deg,rgba(8,22,38,.96),transparent)}.session-title{font-size:14px;font-weight:800}.session-meta{font-size:10px;color:#a9bdd2;margin-top:4px}.slide-count{font-size:11px;color:#b7cbe0}.overlay{position:absolute;z-index:8;left:50%;top:50%;transform:translate(-50%,-50%);width:min(80vw,850px);padding:34px;background:rgba(13,30,50,.96);border:1px solid rgba(137,181,221,.3);border-radius:16px;text-align:center;box-shadow:0 30px 100px #000}.overlay .kicker{font-size:11px;letter-spacing:.15em;color:#91c9f4;font-weight:800}.overlay h2{font-size:clamp(30px,4vw,58px);margin:10px 0;line-height:1.05}.announcement{position:absolute;z-index:9;left:50%;bottom:74px;transform:translateX(-50%);background:rgba(16,37,60,.96);border:1px solid rgba(133,174,211,.25);padding:12px 20px;border-radius:999px;font-size:13px;max-width:80vw;text-align:center}.poll{position:absolute;z-index:9;right:30px;bottom:72px;width:min(380px,38vw);padding:18px;background:rgba(17,40,65,.97);border:1px solid rgba(133,174,211,.25);border-radius:14px}.poll h3{font-size:15px;margin:0 0 10px}.poll-option{padding:7px 0;font-size:11px;color:#c4d1df;border-bottom:1px solid rgba(255,255,255,.08)}@media(max-width:700px){.live-top,.bottom{padding-left:15px;padding-right:15px}.deck{width:100vw;height:72vh;border-radius:0}.live-main{padding-left:0;padding-right:0}.poll{left:15px;right:15px;width:auto}.announcement{bottom:65px}.live-brand{display:none}}
  `}</style>
  {live.displayMode !== 'program' && <div className="overlay"><div className="kicker">{live.displayMode==='blank'?'PLEASE STAND BY':live.displayMode==='technical_pause'?'TECHNICAL PAUSE':'LIVE ANNOUNCEMENT'}</div><h2>{live.emergencyMessage || (live.displayMode==='technical_pause'?'We will resume shortly.':'Please stand by.')}</h2></div>}
