...
const days = [
  { id: 'day1', label: 'DAY 1', date: 'Monday, September 21', sessions: [
  ...
  { id: 'day2', label: 'DAY 2', date: 'Tuesday, September 22', sessions: [
  ...
  { id: 'day3', label: 'DAY 3', date: 'Wednesday, September 23', sessions: [
    ['08:00', 'Ice Breaker', 'Josephine'],
...
export default function Summit2026() {
  ...
  const [parking, setParking] = useState({ day1: [], day2: [], day3: [], day4: [] })
  const [notes, setNotes] = useState(() => localStorage.getItem('summit2026-notes') || '')
  const [showNotes, setShowNotes] = useState(false)
  const [showPoll, setShowPoll] = useState(false)
  const [pollAnswer, setPollAnswer] = useState('')
  const [timerSeconds, setTimerSeconds] = useState(0)
  const [timerRunning, setTimerRunning] = useState(false)
  const [participantName, setParticipantName] = useState(() => localStorage.getItem('summit2026-name') || '')
  const [anonymous, setAnonymous] = useState(false)

  const day = days[dayIndex]
  const hasSessions = day.sessions.length > 0
  const session = hasSessions ? day.sessions[selected] : null
  ...
  useEffect(() => {
    localStorage.setItem('summit2026-notes', notes)
  }, [notes])

  useEffect(() => {
    if (participantName) localStorage.setItem('summit2026-name', participantName)
  }, [participantName])
  ...
  const postParking = () => {
    ...
    setParking((current) => ({
      ...current,
      [day.id]: [...(current[day.id] || []), {
        id: crypto.randomUUID(),
        text,
        name: anonymous ? 'Anonymous' : (participantName.trim() || 'Participant'),
  ...
  const vote = (id) => setParking((current) => ({
    ...current,
  ...
  return (
    <div className="summit-shell">
      ...
      <main className="summit-main">
        ...
        {hasSessions ? <div className="grid">
          <section>
            <div className="card"><div className="card-head"><h2>{day.label} · {day.date}</h2><span className="muted">{day.sessions.length} programme items</span></div>
            ...
            <div className="card" style={{ marginTop: 15 }}><div className="card-head"><h2>Programme Timeline</h2><span className="muted">Select a session</span></div><div className="timeline">{day.sessions.map((item, index) => <button type="button" key={`${item[0]}-${item[1]}`} className={`timeline-row ${index === selected ? 'active' : ''}`} onClick={() => chooseSession(index)} style={{ width: '100%', textAlign: 'left', background: index === selected ? undefined : '#fff', borderTop: 0, borderRight: 0, borderLeft: index === selected ? undefined : 0, borderBottom: '1px solid #edf0f4' }}><div className="time">{item[0]}</div><div className="row-title">{item[1]}</div><div className="fac">{item[2] || '—'}</div></button>)}</div></div>
          </section>

          <aside><div className="card"><div className="card-head"><h2>Run of Show</h2><span className="muted">Coordinator view</span></div><div className="side-section">
            ...
            <div className="metric-label">Session timer</div><div className="timer">{formatTimer}</div><div className="buttons"><button type="button" className="btn primary" onClick={() => setTimerRunning((value) => !value)}>{timerRunning ? 'Pause' : 'Start'}</button><button type="button" className="btn" onClick={() => { setTimerRunning(false); setTimerSeconds(0) }}>Reset</button></div>
            <hr className="rule" /><div className="metric-label">Coordinator controls</div><div className="muted" style={{ lineHeight: 1.5, marginTop: 5 }}>Production controls will include slide override, Parking Lot moderation, announcements, poll management and role-based access.</div>
            <hr className="rule" /><div className="buttons"><button type="button" className="btn gold" onClick={() => setShowNotes((value) => !value)}>My Notes</button><button type="button" className="btn" onClick={() => alert('Announcement channel is ready for backend integration.')}>Announcement</button></div>
        ...
        </div> : <div className="card departure"><h3>September 24 · Departure Day</h3><p className="muted">The programme source identifies September 24 as the departure day. No programme sessions are listed for this date.</p><ul><li>Departure-day workspace</li><li>Coordinator announcements</li><li>Participant notes and final resources</li></ul></div>}

        <div className="card parking"><div className="card-head"><h2>{day.label} Parking Lot</h2><span className="muted">{posts.length} post{posts.length === 1 ? '' : 's'}</span></div><div className="parking-form"><textarea value={parkingInput} onChange={(event) => setParkingInput(event.target.value)} placeholder="Post a question, idea, issue, or topic to revisit..." aria-label="Parking Lot contribution" /><div className="identity"><input type="text" value={participantName} onChange={(event) => setParticipantName(event.target.value)} placeholder="Display name" aria-label="Display name" /><label className="muted"><input type="checkbox" checked={anonymous} onChange={(event) => setAnonymous(event.target.checked)} /> Post anonymously</label></div><div className="parking-actions"><span className="muted">Prototype: posts are local to this browser until realtime backend is connected.</span><button type="button" className="btn primary" onClick={postParking}>Post to Parking Lot</button></div></div>{posts.length === 0 ? <div className="post"><span className="muted">No contributions yet. Start the Parking Lot with a question or idea.</span></div> : [...posts].reverse().map((item) => <article className="post" key={item.id}><div className="post-meta"><span>{item.name}</span><span>{item.time}</span></div><div className="post-text">{item.text}</div><button type="button" className="vote" onClick={() => vote(item.id)}>▲ Upvote {item.votes}</button></article>)}</div>
      </main>
    </div>
  )
}
