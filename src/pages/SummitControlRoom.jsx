...
export default function SummitControlRoom() {
  ...
  const loadPulse = async () => {
    ...
    const [{count:participants},{data:activeRows},{data:attendanceRows},{data:state}] = await Promise.all([
      summitSupabase.from('participants').select('id',{count:'exact',head:true}),
      summitSupabase.from('participants').select('id').gte('last_seen_at',new Date(Date.now()-5*60*1000).toISOString()),
      summitSupabase.from('participant_attendance').select('participant_id').eq('day_id',currentDayId),
      summitSupabase.from('summit_live_state').select('display_mode,emergency_message').eq('id',1).maybeSingle(),
    ])
    setPulse({participants:participants||0,active:activeRows?.length||0,attendance:new Set((attendanceRows||[]).map(x=>x.participant_id)).size})
    if(state){setLiveMode(state.display_mode||'program');setLiveMessage(state.emergency_message||'')}
  }
  const setDisplayMode = async mode => { setLiveMode(mode); await realtime.publish(summitEventTypes.DISPLAY_MODE_CHANGED,{mode,message:liveMessage}) }
  ...
  useEffect(() => { void loadSessions(); void loadLiveData(); void loadPulse(); const id=setInterval(()=>void loadPulse(),10000); return () => { clearInterval(id); realtime.close() } }, [realtime,currentDayId])
  ...
  useEffect(() => realtime.subscribe(event => {
    ...
    if (type === summitEventTypes.PARKING_MODERATION && payload.id) setParking(items => items.map(item=>item.id===payload.id?{...item,visible:payload.visible!==false,pinned:payload.pinned??item.pinned}:item))
  ...
  const toggleParking = id => setParking(items=>items.map(item=>{if(item.id!==id)return item;const visible=!item.visible;realtime.publish(summitEventTypes.PARKING_MODERATION,{dayId:item.dayId,id,visible,pinned:item.pinned});return {...item,visible}}))
  const togglePinned = id => setParking(items=>items.map(item=>{if(item.id!==id)return item;const pinned=!item.pinned;realtime.publish(summitEventTypes.PARKING_MODERATION,{dayId:item.dayId,id,visible:item.visible,pinned});return {...item,pinned}}))
  ...
  `}</style>
  <header className="cr-head"><div className="cr-wrap"><div className="small" style={{color:'#b9c9da'}}>SUMMIT OF AMERICAN SPACES IN NIGERIA 2026 · COORDINATOR</div><h1>Control Room</h1><p>Live operations workspace for sessions, announcements, polls, participant access and Parking Lot moderation.</p><nav className="cr-actions"><a className="cr-action primary" href="/summit-2026/live" target="_blank" rel="noreferrer">Open LIVE SCREEN</a><a className="cr-action" href="/summit-2026/control-room/content">Manage Presentations & Facilitator Profiles</a><a className="cr-action" href="/summit-2026/control-room/participants">Participant Management</a></nav></div></header>
  <div style={{maxWidth:'1500px',margin:'18px auto 0',padding:'0 20px'}}><div className="cr-card"><div className="cr-title"><h2>LIVE BROADCAST CONTROL</h2><span className="status">{liveMode.replace('_',' ').toUpperCase()}</span></div><div className="cr-body"><div className="row" style={{marginTop:0}}><button className="btn ${liveMode==='program'?'primary':''}" onClick={()=>setDisplayMode('program')}>RETURN TO PROGRAMME</button><button className="btn ${liveMode==='technical_pause'?'primary':''}" onClick={()=>setDisplayMode('technical_pause')}>TECHNICAL PAUSE</button><button className="btn ${liveMode==='blank'?'danger':''}" onClick={()=>setDisplayMode('blank')}>BLANK SCREEN</button></div><input className="input" style={{marginTop:8}} value={liveMessage} onChange={e=>setLiveMessage(e.target.value)} placeholder="Optional message for technical pause / screen announcement"/><div className="row"><button className="btn primary" onClick={()=>setDisplayMode('announcement')}>BROADCAST SCREEN MESSAGE</button></div><div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginTop:12}}><div className="metric"><div className="label">Registered</div><strong>{pulse.participants}</strong></div><div className="metric"><div className="label">Active · 5 min</div><strong>{pulse.active}</strong></div><div className="metric"><div className="label">Day Attendance</div><strong>{pulse.attendance}</strong></div><div className="metric"><div className="label">Backend</div><strong style={{fontSize:15}}>{summitSupabase?'ONLINE':'OFFLINE'}</strong></div></div></div></div></div><main className="cr-grid">
