...
const sessions = [
  ...
  ['DAY 2', '08:00', 'Ice Breaker'], ['DAY 2', '08:05', 'Hands-On Session: AI-Assisted Program Planning'], ['DAY 2', '09:15', 'Hands-On Session: AI Assisted Flyer and Graphic Designs'], ['DAY 2', '13:00', 'Hands-On Session: AI for Audience Engagement and Presentation'], ['DAY 2', '14:00', 'Hands-On Session: Use of Gemini NotebookLM'], ['DAY 2', '15:00', 'Mapping ICS Goals to Achieving High Impacting American Spaces Programming'], ['DAY 2', '16:00', 'Hands-On Session: Programming American Spaces using ICS Goals'], ['DAY 2', '16:45', 'Parking Lot and Day 2 Wrap-Up'],
  ['DAY 3', '08:00', 'Ice Breaker'], ['DAY 3', '08:10', 'American Spaces Nigeria Strategic Plan: Review FY2026 and Plan FY2027'], ['DAY 3', '09:10', 'Hands On Session: Introduction to Vibe Coding'], ['DAY 3', '10:25', 'Hands On Session: Introduction to Vibe Coding continued'], ['DAY 3', '13:00', 'Financial matters and Looking Ahead'], ['DAY 3', '13:30', 'Practice Session'], ['DAY 3', '14:40', 'Summit Evaluation'], ['DAY 3', '15:40', 'Closing Ceremony, Certificate Presentation, and Group Photo'], ['DAY 3', '16:30', 'Parking Lot and Day 3 Wrap-Up'],
]
...
export default function SummitControlRoom() {
  ...
  const dayCounts = useMemo(() => ({ 'DAY 1': sessions.filter((item) => item[0] === 'DAY 1').length, 'DAY 2': sessions.filter((item) => item[0] === 'DAY 2').length, 'DAY 3': sessions.filter((item) => item[0] === 'DAY 3').length }), [])

  const broadcast = (type, payload) => createSummitRealtime().publish(type, payload)

  const selectSession = (index) => {
    setSelected(index)
    broadcast(summitEventTypes.SESSION_CHANGED, { dayId: dayIds[sessions[index][0]], sessionIndex: sessions[index][0] === sessions[selected]?.[0] ? index - sessions.slice(0, index).filter((item) => item[0] !== sessions[index][0]).length : 0 })
  }
  ...
  const sendAnnouncement = () => {
    ...
    broadcast(summitEventTypes.ANNOUNCEMENT, { text })
    setAnnouncement('')
  }
  ...
  const publishPoll = () => {
    ...
    setPollOpen(true)
    broadcast(summitEventTypes.POLL_PUBLISHED, { question, options })
  }

  const closePoll = () => { setPollOpen(false); broadcast(summitEventTypes.POLL_CLOSED) }
  const toggleParking = (id) => {
    setParking((items) => items.map((item) => {
      if (item.id !== id) return item
      const visible = !item.visible
      broadcast(summitEventTypes.PARKING_MODERATION, { dayId: dayIds[item.day || sessions[selected][0]], id, visible })
      return { ...item, visible }
    }))
  }
  ...
  return (
    <div className="control-room">
      ...
      <main className="cr-grid">
        <section className="cr-card"><div className="cr-title"><h2>Programme</h2><span className="status">COORDINATOR</span></div><div>{sessions.map((item,index)=><button className={`session ${index===selected?'active':''}`} style={{display:'block',width:'100%',textAlign:'left',border:0}} key={`${item[0]}-${item[1]}`} onClick={()=>selectSession(index)}><b>{item[0]} · {item[1]}</b><div>{item[2]}</div></button>)}</div></section>
        <section>
          <div className="cr-card"><div className="cr-title"><h2>Live Session</h2><span className="small">Realtime prototype</span></div><div className="cr-body"><div className="label">Selected session</div><h2 style={{fontSize:22,margin:'5px 0',color:'#173b68'}}>{session[2]}</h2><div className="small">{session[0]} · {session[1]}</div><div className="metric"><div className="label">Facilitator</div><strong style={{fontSize:15}}>[BIO + HEADSHOT TO BE SUPPLIED]</strong></div><div className="metric"><div className="label">Presenter control</div><strong style={{fontSize:15}}>Coordinator Override Available</strong></div><div className="row"><button className="btn primary" onClick={()=>alert('Slide control event is ready for the production backend.')}>Take Slide Control</button><button className="btn" onClick={()=>alert('Session event is ready for the production backend.')}>Advance Session</button></div><div className="notice">Prototype realtime uses browser BroadcastChannel/local persistence. Production cross-device synchronization remains gated by backend security.</div></div></div>
