...
import { summitSupabase } from './summitSupabase'
...
function rememberEvents(events) {
  ...
  try {
    ...
    // Local persistence is best-effort in fallback mode.
  }
}
...
export function createSummitRealtime() {
  ...
  const persistDatabase = async (message) => {
    ...
    try {
      if (message.type === summitEventTypes.PARKING_POST && message.payload?.post) {
        ...
        const { error } = await summitSupabase.from('parking_lot_posts').insert({
          day_id: p.dayId,
          text: p.text,
          display_name: p.name || 'Participant',
          anonymous: p.name === 'Anonymous',
      ...
      if (message.type === summitEventTypes.POLL_RESPONSE && message.payload) {
        ...
        const { error } = await summitSupabase.from('poll_responses').insert({
          poll_id: p.pollId,
          option: p.option,
          display_name: p.name || 'Participant',
          anonymous: p.name === 'Anonymous',
      ...
      if (message.type === summitEventTypes.POLL_PUBLISHED && message.payload) {
        ...
        const { error } = await summitSupabase.from('polls').insert({
          id: pollId,
          question: p.question,
          options: p.options || [],
          status: 'open',
      ...
      console.error('Summit database persistence failed', error)
    }
  }

  if (summitSupabase) {
    channel = summitSupabase.channel(CHANNEL_NAME, {
      config: { broadcast: { self: false, ack: true } },
    })
    channel
      .on('broadcast', { event: 'summit-event' }, (event) => {
        const message = event?.payload
        if (message?.id) deliver(message)
      })
      .subscribe((status, error) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.error('Summit Realtime channel error', status, error)
        }
      })
    ...
    Promise.all([
      ...
      if (closed) return
      for (const p of parkingResult.data || []) {
        deliver({
          id: `db-parking-${p.id}`,
          type: summitEventTypes.PARKING_POST,
          payload: { post: { id: p.id, dayId: p.day_id, text: p.text, name: p.anonymous ? 'Anonymous' : p.display_name, time: new Date(p.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), votes: p.votes, visible: p.visible, pinned: p.pinned } },
          at: p.created_at,
        })
      }
  ...
  const publish = (type, payload = {}) => {
    ...
    deliver(message)
    void sendSupabase(message)
    void persistDatabase(message)
    return message
  }
  ...
  const close = () => {
    closed = true
    if (channel && summitSupabase) summitSupabase.removeChannel(channel)
    listeners.clear()
    seenIds.clear()
    pending.length = 0
  }
